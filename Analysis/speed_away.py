...
'''
!! TO FINISH: CURRENTLY NOT WORKING !!

> interpolate the split segments to get a correct speed signal
'''
...
for k, cond in enumerate(l_cond):
  ...
  with alive_bar(df.shape[0]) as bar:
    ...
    for i, row in df.iterrows():
      ...
      # Positions
      x0 = df[df['frame'].isin(Isplit), 'x']
      y0 = df[df['frame'].isin(Isplit), 'y']

      x1 = df.loc[df.id==1, 'x']
      y1 = df.loc[df.id==1, 'y']

      # Reconstruction
      X0 = np.full(nT, np.nan)
      Y0 = np.full(nT, np.nan)
      X1 = np.full(nT, np.nan)
      Y1 = np.full(nT, np.nan)
      ...
      if len(Isplit):
        ...
        # Distance
        d = np.full(max(df.frame)+1, np.nan)
